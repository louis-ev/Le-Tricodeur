...
*/  
  ...
  for ( int i=0; i < nombreDeTweets; i++ ) {
    ...
    // récupérer l'année du tweet
    int anneeDuTweet = int( toAnnee.format( dateDesTweets[i] ) );
    
    // on visualise la valeur qu'on récupère
    println( "anneeDuTweet : " + anneeDuTweet);
        
    // et on enlève 2006 pour avoir des valeurs potables
    int anneeDuTweetModifiee = anneeDuTweet - 2006;
    
    // elle varie sur pas beaucoup 
    anneeDuTweetModifiee = anneeDuTweetModifiee * 20;

    // on peut aussi récupérer le mois
    int moisDuTweet = int( toMois.format( dateDesTweets[i] ) );

    // en récupérant les mois, on obtient des entiers de 1 à 12
    int jourDuTweet = int( toJour.format( dateDesTweets[i] ) );
    jourDuTweet = jourDuTweet * 3;

    fill(0);
    noStroke();    
    rect( jourDuTweet, anneeDuTweetModifiee + moisDuTweet, 1, 1 );

    // on légende avec l'année en question
    text( anneeDuTweet, 5, anneeDuTweetModifiee - 1 );    
